          ...
          <div className="flex-1 text-center md:text-left z-10">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-3">
              <span className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-800 rounded-lg text-sm text-gray-300">
                ...
                {user.addresses?.[0]?.address || 'No address set'}
              </span>
              <span className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-800 rounded-lg text-sm text-gray-300">
                    ...
                    {order.items.map((item: any, idx: number) => (
                      <div key={idx} className="flex justify-between items-center">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-gray-300 font-medium">{item.nameEn}</span>
                        </div>
        ...
        {activeTab === 'profile' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-900 border border-gray-800 rounded-3xl p-8">
             ...
             <div className="grid sm:grid-cols-2 gap-6">
               ...
               <div>
                 <label className="block text-gray-500 text-xs mb-1">Phone Number</label>
                 <div className="bg-gray-950 border border-gray-800 text-white px-4 py-3 rounded-xl">{user.phone}</div>
               </div>
               <div>
             ...
             <h2 className="text-xl font-bold text-white mb-4 mt-8">Saved Addresses</h2>
             {user.addresses?.map((addr: any) => (
               <div key={addr.id} className="bg-gray-950 border border-gray-800 p-4 rounded-xl flex items-start gap-4">
                 <div className="p-2 bg-orange-500/10 rounded-lg text-orange-400">
                   <MapPin size={20} />
                 </div>
                 <div>
                   <h4 className="text-white font-bold">{addr.type}</h4>
